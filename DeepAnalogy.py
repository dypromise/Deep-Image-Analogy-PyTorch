...
import numpy as np
import copy
from utils import ts2np, np2ts, blend, normalize
import time
import datetime
...
def analogy(img_A, img_BP, config):
    ...
    # preparing data
    img_A_tensor = torch.FloatTensor(
        img_A.transpose(2, 0, 1)).to(device).unsqueeze(0)
    img_BP_tensor = torch.FloatTensor(
        img_BP.transpose(2, 0, 1)).to(device).unsqueeze(0)
    ...
    for curr_layer in range(5):
        ...
        data_AP[curr_layer] = blend(response_A,
                                    data_A[curr_layer],
                                    data_AP[curr_layer],
                                    weights[curr_layer],
                                    device=device)
        data_B[curr_layer] = blend(response_BP,
                                   data_BP[curr_layer],
                                   data_B[curr_layer],
                                   weights[curr_layer],
                                   device=device)
        ...
        if curr_layer >= 4:
            print("### current stage: %d - end | " % (5-curr_layer)+"Elapse: "
        ...
        data_AP_np = avg_vote(ann_AB_upnnf2,
                              ts2np(data_BP[next_layer]),
                              sizes[next_layer],
                              data_A_size[next_layer][2:],
                              data_B_size[next_layer][2:])
        data_B_np = avg_vote(ann_BA_upnnf2,
                             ts2np(data_A[next_layer]),
                             sizes[next_layer],
                             data_B_size[next_layer][2:],
                             data_A_size[next_layer][2:])
        ...
        target_BP_np = avg_vote(ann_AB,
                                ts2np(data_BP[curr_layer]),
                                sizes[curr_layer],
                                data_A_size[curr_layer][2:],
                                data_B_size[curr_layer][2:])
        target_A_np = avg_vote(ann_BA,
                               ts2np(data_A[curr_layer]),
                               sizes[curr_layer],
                               data_B_size[curr_layer][2:],
                               data_A_size[curr_layer][2:])
        ...
        start_time_2 = time.time()
        data_AP[curr_layer+1] = model.get_deconvoluted_feat_v2(
            target_BP,
            curr_layer,
            data_AP[next_layer],
            lr=lr[curr_layer],
        ...
        start_time_2 = time.time()
        data_B[curr_layer+1] = model.get_deconvoluted_feat_v2(
            target_A,
            curr_layer,
            data_B[next_layer],
            lr=lr[curr_layer],
        ...
        if data_B[curr_layer + 1].type() == torch.cuda.DoubleTensor:
            data_B[curr_layer + 1] = data_B[
                curr_layer + 1].type(torch.cuda.FloatTensor)
            data_AP[curr_layer + 1] = data_AP[
                curr_layer + 1].type(torch.cuda.FloatTensor)

        print("### current stage: %d - end | " % (5-curr_layer)+"Elapse: " +
              str(datetime.timedelta(seconds=time.time() - start_time_1))[:-7]+' ###')

    print('\n####################### reconstruct images A\' and B #########################')
    img_AP = avg_vote(ann_AB, img_BP,
                      sizes[curr_layer],
                      data_A_size[curr_layer][2:],
                      data_B_size[curr_layer][2:])
    img_B = avg_vote(ann_BA, img_A,
                     sizes[curr_layer],
                     data_A_size[curr_layer][2:],
                     data_B_size[curr_layer][2:])
    ...
    return img_AP, img_B, str(datetime.timedelta(
        seconds=time.time() - start_time_0))[:-7]
