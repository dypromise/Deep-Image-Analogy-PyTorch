...
def analogy(img_A, img_BP, config):
    ...
    img_A_tensor, img_BP_tensor = img_A_tensor.to(
        device), img_BP_tensor.to(device)
    ...
    for curr_layer in range(5):
        ...
        data_AP[curr_layer] = blend(
            response_A, data_A[curr_layer], data_AP[curr_layer],
            weights[curr_layer], device=device)
        data_B[curr_layer] = blend(
            response_BP, data_BP[curr_layer], data_B[curr_layer],
            weights[curr_layer], device=device)
        ...
        ann_BA, _ = propagate(ann_BA, ts2np(Ndata_BP), ts2np(Ndata_B),
                              ts2np(Ndata_AP), ts2np(
                                  Ndata_A), sizes[curr_layer],
                              params['iter'], rangee[curr_layer])
        print("\tElapse: " +
              str(datetime.timedelta(seconds=time.time() - start_time_2))[:-7])
        ...
        data_AP[curr_layer+1] = model.get_deconvoluted_feat_v2(
            target_BP, curr_layer, data_AP[next_layer], lr=lr[curr_layer],
            iters=200, display=True, device=device)
        print("\tElapse: " +
              str(datetime.timedelta(seconds=time.time() - start_time_2))[:-7])
        ...
        data_B[curr_layer+1] = model.get_deconvoluted_feat_v2(
            target_A, curr_layer, data_B[next_layer], lr=lr[curr_layer],
            iters=200, display=True, device=device)
        print("\tElapse: " +
              str(datetime.timedelta(seconds=time.time() - start_time_2))[:-7])
    ...
    img_AP = reconstruct_avg(
        ann_AB, img_BP, sizes[curr_layer], data_A_size[curr_layer][2:],
        data_B_size[curr_layer][2:])
    img_B = reconstruct_avg(
        ann_BA, img_A, sizes[curr_layer], data_A_size[curr_layer][2:],
        data_B_size[curr_layer][2:])

    img_AP = np.clip(img_AP, 0, 255)
    img_B = np.clip(img_B, 0, 255)

    return img_AP, img_B, str(datetime.timedelta(seconds=time.time() -
                                                 start_time_0))[:-7]
