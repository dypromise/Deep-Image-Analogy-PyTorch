...
def propagate(nnf, feat_A, feat_AP, feat_B, feat_BP, patch_size, iters=2, rand_search_radius=200):
    print("\tpatch_size:{}; num_iters:{}; rand_search_radius:{}".format(
        patch_size, iters, rand_search_radius))
    ...
    for ay in range(A_size[0]):
        for ax in range(A_size[1]):
            by, bx = nnf[ay, ax]
            nnd[ay, ax] = cal_dist(
                ay, ax, by, bx, feat_A, feat_AP, feat_B, feat_BP, A_size, B_size, patch_size)
...
def cal_dist(ay, ax, by, bx, feat_A, feat_AP, feat_B, feat_BP, A_size, B_size, patch_size, cutoff=np.inf):
    ...
    try:
        if feat_A.shape[2] == 3:
            ...
            dist1 = -np.sum(feat_A[ay - dy0:ay + dy1, ax - dx0:ax + dx1]
                            * feat_B[by - dy0:by + dy1, bx - dx0:bx + dx1])
            dist2 = -np.sum(
        ...
        print(e)
        print("dx0:{}; dx1:{}; dy0:{}; dy1:{}; ax:{}; ay:{}; bx:{}; by:{}".format(
            dx0, dx1, dy0, dy1, ax, ay, bx, by))
    return dist
